...
You can add tags in the `frontmatter{:markdown}` of your MDX file:

```markdown title="add-tags-and-categories-to-gatsby-mdx-blog.md"
---
...
The `frontmatter{:markdown}` is the top area of your Markdown or MDX file that is surrounded by dashes.
...
Now that the `tags{:js}` fields are available in the data layer, we can use `graphql{:js}` to query it.
Try running the following query, which groups posts by `tags{:js}`, locally in your GraphiQL `http://localhost:8000/___graphql`.

```graphql title="Try on GraphiQL"
{
...
Then, modify your `gatsby-node.js{:js}` file with the above query:
...
As you can see on the `gatsby-node.js{:js}` file, we need a tag page template in order to generate individual pages for the tags in your posts.

Let's create a tag template for `/tags/{tag}{:js}`:
...
This page will render a list of all tags (`src/pages/tags.js{:js}`), followed by the number of posts with that particular tag.
