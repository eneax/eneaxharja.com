...
Objects, on the other hand, are mutable, so we can create more of them. Each time the object literal `{}{:js}` is used, a new object value is created, a value that has never existed before.
...
In addition, you can access their properties using the dot notation `.{:js}` or the bracket notation `[]{:js}`:
...
Once again, `function() {}{:js}` is an expression, or a question to our JavaScript universe, that responds by creating a **new** function value whenever we ask it. This is very similar to the way `{}{:js}` creates a **new** object value when it is executed. This means that functions are very special objects in JavaScript.
...
The `(){:js}` at the end of the function call `footballTeam(){:js}` is what makes the function run. In fact, `footballTeam(){:js}` is an expression. It's known as a **call expression**. To answer a call expression, JavaScript runs the code inside our function, and delivers the returned value as the result (in this example, it is `11{:js}`).

Without `(){:js}`, we would have `let footballPlayers = footballTeam{:js}`, which would point `footballPlayers{:js}` to the value that `footballTeam{:js}` is currently pointing to.
Now, `let players = footballTeam(){:js}` means that I am pointing `players{:js}` to the value returned by the function that `footballTeam{:js}` is currently pointing to.

## Conclusion

Objects and functions are values in JavaScript. While primitive values have always existed in the JavaScript universe, objects and functions behave differently and allow us to generate our own values by writing `{}{:js}` and `function() {}{:js}` respectively.
