...
My favorite approach to style React components is _CSS-in-JS_. With styled-components, I can utilise tagged template literals to write actual CSS code to style my components. That's why I thought to combine the Tailwind experience with the React component model.
...
Once `tailwind macro{:js}` is installed, we need to add a `babel-plugin-macros.config.js{:js}` file in the root folder of our project. Then we need to tell `tailwind macro{:js}` to use the `styled-component macro{:js}`.

```js title="babel-plugin-macros.config.js"
module.exports = {
  tailwind: {
...
```js title="src/tailwind.config.js"
module.exports = {
  purge: [],
  theme: {
    extend: {},
...
```jsx showLineNumbers {3,7,11,13} title="src/pages/test.js"
