...
Consider, for example, the `add(){:js}` function:
...
In fact, while computing the output, the `add(){:js}` function only cares about the argument it receives and nothing else. It does not care about other values or functions outside of it. We call this type of function deterministic.
...
As we can see from the example of `Math.random{:js}` and the weather application, not all functions need to be deterministic, but if a function can be deterministic, then it should be.

## Side effects

You have seen up to this point how easy it is to predict the outcome of a deterministic function, such as the `add(){:js}` function. When something is easy to predict, it is also easy for us to understand its **side effects**. The `add(){:js}` function has no side effects, since it does not produce changes that affect the outside world.
...
The `addCar(){:js}` function will add a new car in the `originalCars{:js}` array using the `push(){:js}` method.
This is a perfect example of an impure function and, as we can see from the result printed on the `console{:js}`, it produces side effects.
In fact, the `addCar(){:js}` function is able to add a new car, but it also modifies the `originalCars{:js}` array which is outside its scope.
...
This can be done by replacing the `push(){:js}` method with the `spread operator{:js}`, as shown below:
...
We have seen the `add(){:js}` function in action, but we can look at the _square_ function as well:
...
The `square(){:js}` function above will always give the same result for the same input. Also, by calculating the square of `num{:js}`, it will not change the value of `num{:js}` itself.
