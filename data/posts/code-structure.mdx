...
A statement example is the `alert(){:js}` method, which shows the message "Hello, world!".
...
It starts with two forward slash characters `//{:js}` and anything else that comes after it is a comment.
...
This type of comment starts with a forward slash and an asterisk `/*{:js}` and ends with an asterisk and a forward slash `*/{:js}`.
...
The above code, for instance, will return an error `Uncaught SyntaxError: Unexpected token '*'{:js}`.
