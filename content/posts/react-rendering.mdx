...
1. React creates a snapshot of a component by capturing everything it needs to update the user interface at that particular time. This includes _props_, _state_, _event handlers_, and a _description of the UI_ (based on the props and state);
2. React takes that _description of the UI_ and uses it to update the UI.
